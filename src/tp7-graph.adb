...
--
--
-- NOTES                            : Ada 2005, GTKAda GPL 2016
--
...
package body TP7.Graph is
   ...
   is
      Context : constant Gtk.Style_Context.Gtk_Style_Context :=
        Gtk.Style_Context.Get_Style_Context (Area);
   begin
      Gtk.Style_Context.Render_Background (Context, Cr, 0.0, 0.0, 640.0, 480.0);
      ...
      if MEH /= null then
         ...
         Gtk.Widget.Set_Events
           (Gtk.Widget.Gtk_Widget (Area_Draw),
            Gdk.Event.Button_Press_Mask or Gdk.Event.Button_Release_Mask or Gdk.Event.Scroll_Mask);
      end if;
      Win_Draw.Show_All;
   ...
   begin
      if ColorNum <= IntPalette.Size - 1 then
         ...
         IntColorPalette.Colors (Word (ColorNum)) :=
           (GDouble (RedValue), GDouble (GreenValue), GDouble (BlueValue), 1.0);
         -- TBF activation
         Gdk.Threads.Leave;
      end if;
      if Debug then
