...
package TP7 is
   ...
   procedure Init (My_Principal_Proc : TPProc; Restore_Env : Boolean := True);
   procedure Main_Loop;

   -- Tips : call this function to test if user as checked Debug in Win_Ctrl
   function Debug return Boolean;

   -- Tips : call this procedure before GTKAda (it is a workaroung with running in GPS)
...
private
   ...
   procedure Get_Key_Event
     (Event_Handler : out Gtkada.Handlers.Return_Callback.Event_Marshaller.Handler);
   procedure Init_Graph (InitProc : TPProc);
